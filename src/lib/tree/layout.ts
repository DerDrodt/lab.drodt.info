import type { Layout, Tree, UI } from '$lib/types';
import { maxBy } from '$lib/util/max-by';

// Code taken and adjusted from the paper "Drawing Non-layered Tidy Trees in Linear Time".
// https://doi.org/10.1002/spe.2213

type LayoutItem<T> = Layout.LayoutItem<T>;
type DragTransform = UI.DragTransform;
type LeftSiblingList = Tree.LeftSiblingList;
type Link = Tree.Link;
type Tree<T> = Tree.Tree<T>;
type TreeLayout<T> = Tree.TreeLayout<T>;
 ...
 * @returns {void} - void
 */
export const preOrderTraverseTree = <T>(t: Tree<T>, f: (t: Tree<T>) => void) => {
	f(t);
 ...
 */
export const treeToLayoutItem = <T extends { id: number }>(t: Tree<T>): LayoutItem<T>[] => {
 ...
 */
export const getClosedLeaves = <T extends { closeRef: number | null }>(t: Tree<T>): Tree<T>[] =>
	filterTree(t, (c) => c.data.closeRef !== null);
 ...
 * @returns {void} - void
 */
const nextLeftContour = <T>(t: Tree<T>) => (t.children.length ? t.children[0] : t.tl);
 ...
 * @returns {void} - void
 */
const setLeftThread = <T>(t: Tree<T>, i: number, cl: Tree<T>, modSumCl: number) => {
 ...
 * @returns {void} - void
 */
const setRightThread = <T>(t: Tree<T>, i: number, sr: Tree<T>, modSumSr: number) => {
 ...
 * @returns {void} - void
 */
const distributeExtra = <T>(t: Tree<T>, i: number, si: number, dist: number) => {
	if (si === i - 1) {
		return;
	}
	const nr = i - si;
 ...
 * @returns {void} - void
 */
const iyl = (lowY: number, idx: number, next?: LeftSiblingList): LeftSiblingList => ({
	lowY,
	idx,
	next,
});
 ...
 * @returns {void} - void
 */
const updateIYL = (minY: number, i: number, ih?: LeftSiblingList): LeftSiblingList => {
